...
* **分部位记录与布局**：右下角动作悬浮球上部垂直排列了圆形的部位快捷切换按钮（胸、肩、背、腿、腹），单手极速触达。底部极致精简为“今日总览”与“部位分化”双模式切换栏。
* **无缝状态恢复与云端同步**：自动记住并恢复上次退出的操作页面与选中的部位状态，跨越手机、电脑实现毫秒级数据同步保护。
* **极速免登**：去除了所有账号和登录流程的繁杂概念，即开即练。内部通过硬编码 `87553652@qq.com` 为私有后端身份。
